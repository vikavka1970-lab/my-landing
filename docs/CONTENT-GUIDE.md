...
Основные изменения делаются в `index.html`. Страница тарифов — в `pricing.html`.  
После правки — сохранить и задеплоить: `git add` → `git commit` → `git push`. Полные команды — в разделе «Деплой после изменений» внизу.
...
## Обновить тарифы (pricing.html)

Страница тарифов живёт в отдельном файле `pricing.html`.

Найди нужную карточку через `Ctrl+F` по названию, например: `Лендинг`

```html
<div class="pricing-name">Лендинг</div>
<div class="pricing-price">от 25 000 ₽</div>
```

Поменяй цену или текст внутри карточки.
...
Шаг 1 — добавь изменённые файлы (выбери нужное):

```bash
git add index.html              # правил главный лендинг
git add pricing.html            # правил страницу тарифов
git add index.html images/      # добавил изображение в портфолио
```

Шаг 2 — зафиксируй и отправь:
...
Создаёт новый коммит, который отменяет последний. Безопасно — ничего не удаляется из истории.
...
Отменяет один конкретный коммит, не затрагивая остальные.  
Хэш — первые 7 символов из `git log`, например `aa0c92b`.  
Если нужно отменить несколько коммитов — запусти команду для каждого отдельно.

---

## Что не трогать без понимания

Это правило для любого HTML-файла (`index.html`, `pricing.html` и любых новых):
